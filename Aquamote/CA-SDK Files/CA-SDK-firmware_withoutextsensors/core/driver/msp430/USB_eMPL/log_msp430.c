...
#define min(a,b) ((a < b) ? a : b)
		...
		if (is_uart_enable)
			...
			cdcSendDataInBackground((BYTE*) out, PACKET_LENGTH, CDC0_INTFNUM,
					0);
	}
	va_end(args);
...
void eMPL_send_raw_data(unsigned char type, unsigned char sub_type, long *data) {
	char out[PACKET_LENGTH];
	if (!data)
		return;
	memset(out, 0, PACKET_LENGTH);
