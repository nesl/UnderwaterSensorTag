...
#include "MX25_CMD.h"
#include "msp430_spi.h" // Added for CA-SDK
#include "MX25_DEF.h"   // Added for CA-SDk
/*
 --Common functions
 */
...
void Initial_Spi()
{
    ...
    CSn = 1;        // Chip Select
#endif
	/* Initialize the SPI ports */
	msp430_init_spi_usb1();
    CS_HIGH;
	/* Wait flash warm-up       */
    Wait_Flash_WarmUp();
}
...
void InsertDummyCycle( uint8 dummy_cycle )
{
    ...
    unsigned int loop;
    unsigned int i;
    loop = (int) dummy_cycle/8;
    for(i=0;i<loop;i++){
    	msp430_spi_send(0x00); // Data is 0, only the clock is switching
    }
#endif
}
...
#ifdef MSP430_SPI
    msp430_spi_send(byte_value); // Using msp430_spi_send(data);
#endif
        break;
#endif
#ifdef DIO
...
#ifdef MSP430_SPI
    data_buf = 0;
    set_spi_rx_interrupt_flag(0);
    msp430_spi_send(dummy_val); // Using msp430_spi_send(data);
    while(get_spi_rx_interrupt_flag()==0);
    data_buf=UCB1RXBUF;         // Read the value
#endif
        break;
#endif
#ifdef DIO
