...
typedef enum {
	STATE_WAITING, STATE_READING, STATE_WRITING
} msp430_i2c_state;
...
static msp430_i2c_info i2c = { .enabled = 0 };
...
int msp430_i2c_enable(void) {
	...
	/* Set to synchronous master mode. */CTL0 = UCMST + UCMODE_3 + UCSYNC;

	/* Use sub-master clock. */CTL1 = UCSSEL_2 + UCSWRST;
	...
	/* Enable interrupts. */IE |= UCTXIE | UCRXIE | UCNACKIE;
...
int msp430_i2c_write_new(unsigned char slave_addr,
	...
	i2c.state = STATE_WRITING;
	i2c.slave_reg = reg_addr;
	i2c.slave_reg_written = slave_reg_option;
	i2c.data = (unsigned char*) data;
	i2c.length = length;
                    ...
                    unsigned char *data)
{
    ...
    msp430_get_clock_ms(&start);
    while(CTL1 & UCTXSTT){ // until the 1, stay here.
        msp430_get_clock_ms(&cur);
        if (cur >= (start + I2C_TIMEOUT_MS)) {
            CTL1 = UCTR | UCTXSTP;
            msp430_delay_ms(10);
    ...
    CTL1 |= UCTXSTP;
    msp430_delay_ms(1); // make sure there is enough delay.
    return 0;
}

void msp430_i2c_recover() {
	UCB0CTL1 |= UCSWRST;
	P3SEL &= ~0x03; // Make it I/O
	P3DIR |= 0x03;
	int i = 0;

	// Clock up for 9-cycles at least
	for(i = 0; i < 9; i++){
		P3OUT ^= BIT1;
		msp430_delay_ms(20);
		// Look for SDA to be high to recover I2C
		if( P3OUT & BIT0)
			break;
	}
	P3SEL |= 0x03; // SDA can be in put or output
	P3DIR |= 0x02; // Clock as output
	UCB0CTL1 &= ~UCSWRST;
}
...
__interrupt void I2C_ISR(void) {
	...
	case 10: /* RX interrupt. */
		...
		*i2c.data = RXBUF;
		--i2c.length;
		if (i2c.length) {
			/* If only oen byte left, prepare stop signal. */
			if (i2c.length == 1)
				CTL1 |= UCTXSTP;
			i2c.data++;
		} else {
