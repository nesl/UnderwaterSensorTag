...
# Author: Leon Kozinakov and Eunsun Lee
import sys
import struct
import binascii
...
startpart = 0
imucountlength = (284+2+8)*2
gpscountlength = (30+2)*2
pagecount = 0
time = 1
gpsfix = 0
	...
	if(linecounter3 > linecounter2):
		...
		if(type_of_page == 1):
			...
			if(runOnce == 0):
				...
				outputfile.write("\n")
				outputfile.write("Time" + "," + "AccelX" + "," + "AccelY" + "," + "AccelZ" +"," + "GyroX" + "," + "GyroY" + "," + "GyroZ" + "," + "MagX" + "," + "MagY" + "," + "MagZ"+ "," + "Pressure" + "," + "Temp" +"," + "UTCtime"+ ","+"Latitude"+","+"LatitudeDirection" + "," + "Longitude" + "LongitudeDirection\n")
				runOnce = 1
		...
		if(type_of_page == 2):
			...
			if(concatenatedLines == 16):
				...
				while(pageread):
					...
					if (concatenatedPage[startpart:startpart+4] == '2349'):
						# print("imu")
						imustring = concatenatedPage[startpart:startpart+imucountlength]
						# print(imustring)
						imustring = imustring[4:]

						# time 
						timecount = int(concatenatedPage[0:4], 16)
						imustring = imustring[4:]
						...
						for y in range(0,2):
							for z in range(0,4):
								...
								if (time):
									...
									time = 1
								outputIMU(imustring[0:12], sensitivity_accel)
								imustring = imustring[12:]
								#G
								outputIMU(imustring[0:12], sensitivity_gyro)
								imustring = imustring[12:]
								if(z <= 2):
									if (gpsfix):
								...
								if(gpsfix):
									if(len(my_array)>5):
										# print(my_array)
										outputfile.write(my_array[1]+ "," + my_array[2]+","+my_array[3]+","+my_array[4]+","+my_array[5]+",\n" )
										gpsfix = 0
									else:
										print(my_array)
										outputfile.write("\n")
								...
								outputfile.write("")
							#M
							outputMagnetometer(imustring[0:12], sensitivity_mag)
							imustring = imustring[12:]
						...
						pressureValue = int((imustring[2:4] + imustring[0:2] + imustring[6:8]), 16)
						temperatureValue = int((imustring[4:6] + imustring[10:12] + imustring[8:10]), 16)
						...
						#seconds
						the_seconds = float(int(imustring[0:8], 16))
						imustring = imustring[8:]
						#fraction
						decimal_time = the_seconds + float(int(imustring[0:8], 16)) / (4294967295)
						imustring = imustring[8:]
						outputfile.write(str(decimal_time) + ",")
						time = 0


						startpart = startpart + imucountlength
					elif (concatenatedPage[startpart:startpart+4] == '2346'):
						# print("gps")
						gpsstring = concatenatedPage[startpart:startpart+gpscountlength]
						# print(gpsstring)
						gpsstring = binascii.unhexlify(gpsstring).decode("utf-8")
						print(gpsstring)

						# gpsstring = "#F,123519,4807.038,N,01131.000,E,1,08,0.9,545.4,M,46.9,M,,*47"
						my_array = gpsstring.split(',')
						print(my_array)
						# UTCtime = my_array[1]
						# print("UTCtime: ", UTCtime)
						# Latitude0 = my_array[2:4]
						# print("Latitude: ", Latitude)
						# Longitude = my_array[4:6]
						...
						startpart = startpart + gpscountlength
					elif (concatenatedPage[startpart:startpart+4] == '0000'):
						pageread = 0
						startpart = 0
						# print("endofpage")
